    ...
    def get(self, path: str) -> bytes:
        request = urllib.request.Request(
            ...
            headers={"Accept": "application/vnd.github+json", "Authorization": f"Bearer {self.token}",
                     "User-Agent": "workflow-diagnostics", "X-GitHub-Api-Version": API_VERSION},
        )
        try:
    ...
    for job in jobs:
        ...
        for step in job.get("steps", []) if isinstance(job.get("steps"), list) else []:
            if isinstance(step, Mapping):
                steps.append({"number": step.get("number"), "name": step.get("name"), "status": step.get("status"),
                              "conclusion": step.get("conclusion"), "started_at": step.get("started_at"),
                              "completed_at": step.get("completed_at"),
                              "duration_seconds": duration(step.get("started_at"), step.get("completed_at"))})
        output.append({"id": job.get("id"), "name": job.get("name"), "status": job.get("status"),
...
def classify(lines: list[str], conclusion: str, jobs: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    failed_steps = [{"job_id": job["id"], "job_name": job["name"], "step_number": step["number"],
                     "step_name": step["name"], "conclusion": step["conclusion"]}
    ...
    return {"primary_category": primary, "candidates": candidates[:5], "failed_steps": failed_steps,
...
def main() -> int:
    ...
    try:
        ...
        for run in client.recent_runs(cutoff_dt, args.max_runs):
            if str(run.get("path") or "") in excluded:
                continue
            ...
            record = {"id": run_id, "name": run.get("name"), "path": run.get("path"), "event": run.get("event"),
                    ...
                    for path in paths:
                        for line in path.read_text(encoding="utf-8", errors="replace").splitlines():
                            if KEY_LINE.search(line) and line not in seen:
                                seen.add(line); lines.append(line[:2000])
                                if len(lines) >= 300:
                                    break
