...
class ReconciliationIdentityTests(unittest.TestCase):
    ...
    def test_rejects_non_control_title(self) -> None:
        self.assertIsNone(
            MODULE.reconciliation_candidate(self.issue(title="[audit]"))
        )

    def test_rejects_non_owner_issue(self) -> None:
        self.assertIsNone(
            MODULE.reconciliation_candidate(self.issue(actor="attacker"))
        )
    ...
    def test_rejects_child_repository_not_fixed_by_route(self) -> None:
        self.assertIsNone(
            MODULE.reconciliation_candidate(
                self.issue(
                    child_repository="a15280020511/expert-assessment-center"
                )
            )
        )
    ...
    def test_reconcile_uses_deferred_terminal_validator(self) -> None:
        self.assertIsNot(
            MODULE.DEFERRED.trusted_terminal,
            MODULE.CONTROL._trusted_terminal,
        )
