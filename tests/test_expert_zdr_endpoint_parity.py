...
class ExpertZdrEndpointParityTests(unittest.TestCase):
    def test_public_endpoint_without_authenticated_zdr_route_is_skipped(self) -> None:
        ...
        def fake_fetch(url: str, token: str):
            ...
            if "solar-pro-3" in url:
                return {"data": {"endpoints": [endpoint("ordinary-provider")]}}
            return {"data": {"endpoints": [endpoint("zdr-provider")]}}
        ...
        self.assertEqual(nex["qualified_provider_count"], 1)
    ...
    def test_plan_records_zdr_qualification_and_recomputes_digest(self) -> None:
        selector = load_selector()
        rows = [
