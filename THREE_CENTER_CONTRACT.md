...
3. 治理控制平面只允许：创建目标中心正式 Issue、为专家票据发布现有正式命令、读取受信任 Issue 状态、写治理回执和发送摘要通知。
4. 治理控制平面不得执行数据请求、计算、专家研判、业务 Artifact 下载或业务结果修改。
...
9. `CONTROL_PLANE_TOKEN` 仅允许三个业务仓库 Issues 读写，不得拥有业务 Contents 写权限；业务 Secret 和 Server酱 SendKey 不得写入票据。
